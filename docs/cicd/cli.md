...
mv "$(go env GOPATH)/bin/cli" "$(go env GOPATH)/bin/miabi"   # Go names the binary after the module: `cli`
```
...
Or log in once for interactive use. `miabi login` stores the URL + token in `~/.miabi/config.yaml`
(mode `0600`):

```bash
miabi login --server https://miabi.example.com   # opens the browser to sign in (password or SSO)
miabi login --no-browser                         # no local callback: paste a token from /request-token
miabi --token "$MIABI_TOKEN" login               # non-interactive: validate and save this token
```

By default `login` opens the console's `/cli/authorize` page and captures the token through a
one-time code sent to a local callback — nothing to copy and paste. When a token is already set with
...
Each login is saved as a named **context** — server URL, TLS trust, token, and the bound
workspace/app — named after the server's hostname unless you pass `--context <name>`.

```bash
miabi context ls                     # all contexts; the arrow marks the current one  (alias: ctx)
miabi context current                # print the current context's name
miabi context use staging            # switch
miabi context delete staging         # remove (alias: rm)
miabi --context prod apps ls         # target one context for a single command
```

`MIABI_CONTEXT` does the same as `--context`. For a server with a private CA, pass
`--certificate-authority <bundle>` (or `MIABI_CA`); login saves the bundle path with the context.
...
part of the current context in `~/.miabi/config.yaml`:

```bash
miabi workspace list                 # name, display name, role  (alias: ws)
...
| `miabi apps create <name> (--image <img> [--tag] \| --git-repo <url> [--git-ref] [--build-method]) [--port] [--node <id>] [--use]` | Create an app from an image or Git source. `--build-method` is `auto`, `dockerfile`, or `buildpack`. |
| `miabi apps deploy [app] [--tag <tag>] [--strategy <s>] [--no-cache] [--wait] [--timeout 10m]` | Deploy; `--wait` blocks and exits non-zero on failure. `--no-cache` rebuilds every layer (Git-source apps). |
...
| `miabi apps set-source [app] (--image <img> [--tag] [--registry-id <id>] \| --git-repo <url> \| --git-repository-id <id>) [--git-ref] [--build-method] [--builder] [--yes]` | Switch the app between a prebuilt image and a Git build, or edit its current source. Keeps domains, env, volumes and history. `--git-repository-id` uses a saved repository and its credentials; `--yes` skips the prompt when switching source type. |
...
| `miabi apps invalidate-cache [app]` | Start a new build-cache generation, so the next build rebuilds every layer. |
| `miabi apps rm [app] [--yes]` | Delete an application. |
| `miabi pipeline ls` | List the workspace's [pipelines](/docs/cicd/pipelines). |
| `miabi pipeline run <pipeline> [--branch <ref>] [--commit <sha>] [--no-cache]` | Start a run (pipeline by name, UID, or id). |
| `miabi pipeline rerun <run-id> [--no-cache]` | Run the same pipeline again at the ref and commit that run built. |
| `miabi volumes …` | Manage volumes, their files, and their backups (see below). |
...
Persistent [volumes](/docs/storage/volumes), addressed by **name** (alias: `vol`).

```bash
miabi volumes ls
miabi volumes get web-data                    # details and the apps mounting it
...
miabi volumes attach web-data --path /var/lib/data [--app web]
miabi volumes detach web-data [--app web]     # unmount; the data is kept
miabi volumes storage                         # workspace usage: declared vs measured
miabi volumes rm web-data [--yes]             # refused while an app mounts it
```

`--driver` is `local` (the default: node-local, one node mounts it), `nfs` or `cifs` (shared storage
every replica can mount), or `host` (an operator-managed path, privileged workspaces only). Driver
options are write-only; pass anything holding a password with `--driver-opt-file key=path` (`-` reads
stdin) to keep it out of your shell history.

Files inside a volume — one side of `cp` is `<volume>:<path>`, and `-` is stdin/stdout:

```bash
miabi volumes ls-files web-data [--path uploads]
miabi volumes cp ./nginx.conf web-data:/conf/nginx.conf       # upload (overwrites)
miabi volumes cp web-data:/dump.sql - | gzip > dump.sql.gz   # download; --force overwrites a local file
miabi volumes rm-file web-data uploads [--yes]               # a directory is removed recursively
```

The panel rejects uploads over 512 MiB, and listings stop at 5000 entries.

Backups go to the workspace's S3 [backup target](/docs/storage/backup-targets); without one every
backup command is refused. Backups are addressed by the numeric ID the listing shows.

```bash
miabi volumes backups web-data                        # list
miabi volumes backups run web-data [--wait] [--timeout 30m]
miabi volumes backups restore web-data 42 [--yes]     # overwrites the volume's data
miabi volumes backups logs web-data 42
miabi volumes backups rm web-data 42 [--yes]
```

`restore` does not stop the apps mounting the volume — stop them first unless the workload tolerates
its files changing underneath it.
...
with `--from-file` (or stdin) rather than `--value`, which lands in your shell history.
...
miabi secrets set API_KEY --description "Stripe live key"   # update only the description
...
(`[key=]path`, repeatable) or `--from-dir`. `--mode` sets the default octal file mode (e.g. `0644`),
`--delimiters "<<,>>"` changes the interpolation delimiters for files whose own syntax uses `{{ }}`,
and `--description` labels the config.
...
- `--context <name>` — use a saved context for one command (env `MIABI_CONTEXT`).
- `--certificate-authority <file>` — trust a CA bundle for the server's certificate (env `MIABI_CA`).
- `--insecure-skip-tls-verify` — skip certificate verification (env `MIABI_INSECURE_SKIP_TLS_VERIFY`).
  The CLI warns on every command that uses it; prefer `--certificate-authority`.
- `--no-color` — plain output (also auto-disabled off a TTY or when `NO_COLOR` or `MIABI_NO_COLOR` is
  set).
...
Docker host. `-f, --file` points at a manifest other than `/etc/miabi/miabi.yaml`; so do
`MIABI_CONFIG_FILE` and the older `MIABI_STACK_FILE`.

`setup` also takes `--admin-email`, `--acme-email`, `--control-url`, `--registry`,
`--registry-host`, `--subnet`, `--internal-subnet`, `--gateway-image`, `--runner-image`,
`--goma-config`, and `--no-host-proc`. Each one overwrites the matching manifest field when given
and leaves it alone otherwise — see [Install options](/docs/getting-started/installation#install-options).

A **fresh** `setup`, and `upgrade` with no component, install the latest published Miabi release,
resolved from GitHub when the command runs — the CLI does not carry a platform version, because it
releases on its own cadence. Re-running `setup` on an existing install keeps the image its manifest
pins. `MIABI_RELEASE_API` points that lookup at a mirror for a host with no route to GitHub, and
`--version`/`--image` skip it entirely.

On `upgrade`, `--version` accepts `1.8.0` or `v1.8.0` and swaps only the tag on the component's
current reference, so a private registry and non-Miabi components (the gateway) keep working;
`--image` replaces the reference outright. On `setup`, `--version` sets the control-plane image to
`miabi/miabi:<version>`, replacing a private-registry reference — pass `--image` there, or use
`upgrade`. Pinning matters: a floating tag such as `latest` warns, because a failed rollout has no
distinct previous image to roll back to. See [Upgrades](/docs/upgrades/upgrading).
...
- name: Deploy to Miabi
  run: |
    docker run --rm -e MIABI_SERVER -e MIABI_TOKEN \
      miabi/cli:latest apps deploy web --tag "${{ github.sha }}" --wait
  env:
    MIABI_SERVER: ${{ vars.MIABI_SERVER }}
    MIABI_TOKEN:  ${{ secrets.MIABI_DEPLOY_TOKEN }}
```

Because `--wait` exits non-zero on a failed rollout, a broken deploy fails the pipeline step. (A
[prebuilt release binary](https://github.com/miabi-io/cli/releases/latest) works the same way if the
runner has no Docker.)
...
The request body accepts `registry_id`, `tag`, `strategy`, and `no_cache` (rebuild every layer, for
Git-source apps). There is no `wait` field and no
...
- [Volumes](/docs/storage/volumes) — the storage managed by `miabi volumes`.
...
- [Pipelines](/docs/cicd/pipelines) and [Deploy on push](/docs/cicd/git-push-deploy) — in-platform
